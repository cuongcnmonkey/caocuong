...
bool GameScene1::init()
{
    ...
    listener->onTouchBegan = CC_CALLBACK_2(GameScene1::onTouchBegan, this);
    listener->onTouchMoved = CC_CALLBACK_2(GameScene1::onTouchMoved, this);
    listener->onTouchEnded = CC_CALLBACK_2(GameScene1::onTouchEnded, this);
    listener->onTouchCancelled = CC_CALLBACK_2(GameScene1::onTouchCancelled, this);
...
bool GameScene1::onTouchBegan(cocos2d::Touch* touch, cocos2d::Event* event)
{
    return true; // true if the function wants to swallow the touch
}

void GameScene1::onTouchMoved(cocos2d::Touch* touch, cocos2d::Event* event)
{

}


void GameScene1::onTouchEnded(cocos2d::Touch* touch, cocos2d::Event* event)
{
...
void GameScene1::onTouchCancelled(cocos2d::Touch* touch, cocos2d::Event* event)
{

}

void GameScene1::InitGridRects()
{
...
void GameScene1::InitGridPieces()
{
    for (int x = 0; x < 3; x++)
    {
        for (int y = 0; y < 3; y++)
...
void GameScene1::CheckAndPlacePiece(cocos2d::Touch* touch)
{
        ...
        for (int y = 0; y < 3; y++)
        {
            if (gridSpaces[x][y].containsPoint(touchPoint))
            {
                if (gridArray[x][y] == EMPTY_PIECE)
                {
                    ...
                    gridPieces[x][y]->runAction(Sequence::create(FadeIn::create(PIECE_FADE_IN_TIME), CallFunc::create(std::bind(&GameScene1::CheckWin, this, x, y)), NULL));
                }
            }
        }
    }
...
void GameScene1::CheckWin(int x, int y)
{
...
void GameScene1::Check3PiecesForMatch(int x1, int y1, int x2, int y2, int x3, int y3)
{
