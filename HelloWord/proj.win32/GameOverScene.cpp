...
bool GameOverScene::init()
{
    ...
    auto charts = MenuItemImage::create("char.png", "char.png",
        CC_CALLBACK_1(GameOverScene::DiemCao, this));
    charts->setPosition(Point(1136/2,768/2));

    auto menu = Menu::create(retryItem, mainMenuItem, charts, NULL);
    ...
    currentScore->setPosition(Point(visibleSize.width/2 + origin.x, visibleSize.height / 2 + origin.y + 100));
    ...
    highScoreLabel->setPosition(Point(visibleSize.width/2 + origin.x, visibleSize.height / 2 + origin.y - 100));
...
void GameOverScene::DiemCao(cocos2d::Ref* sender)
{
    auto scene = DiemCao::createScene();
