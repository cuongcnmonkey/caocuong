...
//void BoardView::setScore1(int s) {
//	score1 = s;
//}
//
//int BoardView::getScore1() {
//	return score1;
//}
// Getter
...
Layer* BoardView::createBoardView(Board* board, cocos2d::Layer* layer)
{
	auto boardView = BoardView::create();
	boardView->board = board;
	boardView->showBoard(layer);
	return boardView;
}

void BoardView::showBoard(cocos2d::Layer* layer)
{
	auto visibleSize = Director::getInstance()->getVisibleSize();
	squareSize = visibleSize.width / (board->getNColumns() + 6);
...
bool BoardView::onTouchPokemon(Touch* touch, Event* event) {
	...
	if (target->getBoundingBox().containsPoint(touchLocation)) {
		...
		//	removePokemon1(board->_x, board->_y);
		//	removePokemon1(p.first, p.second);
			board->_x = board->_y = -1;
			...
			auto label = Label::createWithTTF(std::to_string(score1), "fonts/Marker Felt.ttf", 100);
			label->setColor(Color3B::RED);
			label->setPosition(Point(760, 400));
			label->setName("Score");
			this->getChildByName("Score");
			if (this->getChildByName("Score") != NULL) removeChild(this->getChildByName("Score"));
...
void BoardView::connectPokemons(int x, int y, int _x, int _y) {
	...
	auto pokemonFade1 = TargetedAction::create(pokemons[x][y], FadeOut::create(2));
	auto pokemonFade2 = TargetedAction::create(pokemons[_x][_y], FadeOut::create(2));
...
void BoardView::createChoosePokemonEffect(Node* pokemon)
{
	...
	AudioPikachu::playSelectPokemonSoundEffect();
...
{
	AudioPikachu::playRemovePokemonSoundEffect();
