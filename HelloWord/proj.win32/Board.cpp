...
Board::Board(int n_rows, int n_columns, int n_types, vector<int> count) :
	n_rows(n_rows), n_columns(n_columns),
	_pokemons(vector<vector<int>>(n_rows, vector<int>(n_columns, -1)))
{
...
{
	//INIT Graph
	vector<vector<int>> e(n_rows + 2, vector<int>(n_columns + 2, 0));
	for (int i = 0; i < n_rows; ++i)
	{
		for (int j = 0; j < n_columns; ++j)
		{
